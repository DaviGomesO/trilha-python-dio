...
from models_with_POO import Deposit, Withdrawal, NaturalPerson, CurrentAccount
...
def mostrar_extrato(clientes):
    ...
    type_transaction = input(
        "Informe o tipo de transação para o extrato (D-Depósito, S-Saque ou Enter para todos): "
    ).upper()

    report = conta.history.generate_report(
        type_transaction if type_transaction in ["D", "S"] else None
    )
...
def criar_conta_corrente(clientes, contas, numero_conta):
    ...
    if cliente_cadastrado:
        nova_conta = CurrentAccount.new_account(
            client=cliente_cadastrado, number=numero_conta
        )
        contas.append(nova_conta)
        cliente_cadastrado.add_account(nova_conta)
        print(
            f"Conta criada com sucesso! Agência: {nova_conta.agency}, Conta: {nova_conta.number:04d}"
        )
        return True
    else:
